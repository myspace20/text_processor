...
public class BatchProcessor<T> implements Iterator<List<T>> {
    private final int batchSize;
    private List<T> currentBatch;
    private final Iterator<T> iterator;
    private BatchProcessor(Iterator<T> sourceIterator, int batchSize) {
    ...
    public static <T> Stream<List<T>> batchStreamOf(Stream<T> stream, int batchSize) {
        return stream(new BatchProcessor<>(stream.iterator(), batchSize));
    }
